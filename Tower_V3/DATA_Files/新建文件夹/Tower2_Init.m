function Tower0=Tower2_Init(GLB)
...
Ndata  =[{1  0 +0.0 11} {'Cir1001_S1_T0002'}; %POLE(Common node: Tower_Pole)    
         {2  0 -0.4 10} {'Cir3001_A1_T0002'}; %H1_PA1 (Common node: 10kV_A1)
         {3  0 +0.1 10} {'Cir3001_B1_T0002'}; %H1_PB1 (Common node: 10kV_B1)
         {4  0 +0.6 10} {'Cir3001_C1_T0002'}; %H1_PB1 (Common node: 10kV_C1)
...
NodeW.com=NodeW.list(1:4);                 % connected to OHL (SW+3XHV)
NodeW.comdex=NodeW.listdex(1:4,1);
[blen, n0]=size(Wire);
...
if isempty(GridP)
    Ndatag=[{13 0 +0.0 0}  {'GD_PO1_0'};    % Pole gnd electrode (middel)
            {14 0 +0.0 -1} {'GD_PO1_1'};    % Pole gnd electrode (middel)
            {15 0 -2.0 -1} {'GD_PO1_2'};    % Pole gnd electrode (left)
            {16 0 +2.0 -1} {'GD_PO1_3'}];   % Pole gnd electrode (right)
    Ndata=[Ndata; Ndatag];
    tmp=Ndata(:,1:4); Npos=cell2mat(tmp);
    list=string(Ndata(:,5));
    NodeW.list=list;
...
% Nodes 31 and 32 are surf. nodes given in the air group
    Wireg= [{13 14 Npos(13,2:4) Npos(14,2:4)} {'PO_G1'};
            {14 15 Npos(14,2:4) Npos(15,2:4)} {'PO_G2'};
            {14 16 Npos(14,2:4) Npos(16,2:4)} {'PO_G3'}];
    Wire=[Wire; Wireg];
    [b0len, n0]=size(Wire);
    BranW.num = [b0len blen b0len-blen 0 0 0];   
    
    NodeF.com=list(12:13);          % lump elements for wire gnd onnection
    NodeF.comdex=Npos(12:13,1);     % half of air nodes and half gnd nodes
...
data={'Tower No. 2' 1  1 0 [900 0] 0  0 1};
