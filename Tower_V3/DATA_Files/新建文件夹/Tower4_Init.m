function Tower0=Tower4_Init(GLB)
...
Ndata  =[{1  0 +0.0 11} {'Cir1001_S1_T0004'}; %POLE(Common node: Tower_Pole)    
         {2  0 -0.4 10} {'Cir3001_A1_T0004'}; %H1_PA1 (Common node: 10kV_A1)
         {3  0 +0.1 10} {'Cir3001_B1_T0004'}; %H1_PB1 (Common node: 10kV_B1)
         {4  0 +0.6 10} {'Cir3001_C1_T0004'}; %H1_PB1 (Common node: 10kV_C1)
         {5  0 -0.5 8}  {'Cir4001_A1_T0004'}; %L1_PA1 (Common node: 0.4kV_A1)
         {6  0 -0.2 8}  {'Cir4001_B1_T0004'}; %L1_PB1 (Common node: 0.4kV_B1)
         {7  0 +0.1 8}  {'Cir4001_C1_T0004'}; %L1_PB1 (Common node: 0.4kV_C1)
         {8  0 +0.4 8}  {'Cir4001_N1_T0004'}; %L1_PB1 (Common node: 0.4kV_N1)
...
NodeW.com=list(1:8);                 % connected to OHL (SW+3XHV+4xLV)
NodeW.comdex=Npos(1:8,1);
[blen, n0]=size(Wire);
...
if isempty(GridP)
    Ndatag= [{34 0 +0.0 0} {'GD_PO1_0'};    % Pole gnd electrode (middel)
            {35 2 +0.1  0} {'GD_TX1_0'};    % Pole gnd electrode (middel)
            {36 0 +0.0 -1} {'GD_PO1_1'};    % Pole gnd electrode (middel)
            {37 0 -2.0 -1} {'GD_PO1_2'};    % Pole gnd electrode (left)
            {38 0 +2.0 -1} {'GD_PO1_3'};    % Pole gnd electrode (right)
            {39 2 +0.1 -2} {'GD_TX1_1'}];   % TX ground electrode
    Ndata=[Ndata; Ndatag];
    tmp=Ndata(:,1:4); Npos=cell2mat(tmp);
    list=string(Ndata(:,5));
    NodeW.list=list;
...
% Nodes 31 and 32 are surf. nodes given in the air group
    Wireg= [{34 36 Npos(34,2:4) Npos(36,2:4)} {'PO_G1'};
            {36 37 Npos(36,2:4) Npos(37,2:4)} {'PO_G2'};
            {36 38 Npos(36,2:4) Npos(38,2:4)} {'PO_G3'};
            {35 39 Npos(35,2:4) Npos(39,2:4)} {'TX_G1'}];
    Wire=[Wire; Wireg];
    [b0len, n0]=size(Wire);
...
data={'Tower 4' 1  1 0 [1200 0] 0 0 1};
