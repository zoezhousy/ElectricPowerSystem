...
def current_waveform_generator(Wave_Model, DSave, resultstro, resultcur, foldname):
    # struct获取每个变量名
    way = Wave_Model
    ...
    if userInput4 == 1 and way == 1:
        ...
        filepath = foldname + '/' + 'Cigre Function.txt'
        ...
        # 将 DataFrame 保存为 Excel 文件
        df4.to_excel(outputFile, index=False)
        filepath = foldname + '/' + 'Heidler Function.txt'
        # 打开文件并写入
        with open(filepath, 'w') as file:
            file.write(f"syms t\n")
