function [SER, rmserr, bigHfit, opts] = VFdriver_Ding(bigH, s, poles, opts)
...
%           (FRVF) as implemented in vectfit3_Ding.m. All matrix elements are fitted with a common
%           pole set.
% ==========================================================================
% INPUT :
%
...
if isempty(poles)
    ...
    if length(opts.poletype)==8
        ...
        if strcmp(opts.poletype,'lincmplx') %Complex, linearly spaced starting poles
    ...
    if length(poles)<N %An odd number of poles was prescribed
        if strcmp(opts.poletype,'lincmplx')
            pole_extra=-(s(1)/1i+s(end)/1i)/2; %Placing surplus pole in midpoint
        elseif strcmp(opts.poletype,'logcmplx') || strcmp(opts.poletype,'linlogcmplx')
            pole_extra=-10^((log10(s(1)/1i)+log10(s(end)/1i))/2); %Placing surplus pole in midpoint
        end
...
if Nc>1 %Will do only for multi-terminal case
    ...
    for iter=1:Niter1
        ...
        [SER,poles,rmserr,fit]=vectfit3_Ding(f_sum,s,poles,weight_sum,VF);
    end
end
...
for iter=1:Niter2
    ...
    [SER,poles,rmserr,fit1]=vectfit3_Ding(f,s,poles,weight,VF);
end
if Niter2==0
...
if remove_HFpoles==1
    ...
    ind= abs(poles)>factor_HF*abs(s(end)) ;
    ...
    [SER,poles,rmserr,fit2]=vectfit3_Ding(fit1,s,poles,weight,VF);
end
%===========================================
...
if passive_DE==1 && VF.asymp>1
    ...
    for iter=1:1
        ...
        [SER,poles,rmserr,fit3]=vectfit3_Ding(fmod,s,poles,weight,VF);
    end
    SER.D=Dmod;
    SER.E=Emod;
