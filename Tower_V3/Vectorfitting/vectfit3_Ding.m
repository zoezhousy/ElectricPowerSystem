function [SER, poles, rmserr, fit, opts] = vectfit3_Ding(f, s, poles, weight, opts)
%
%function [SER,poles,rmserr,fit,opts]=vectfit3_Ding(f,s,poles,weight,opts)
%function [SER,poles,rmserr,fit]=vectfit3_Ding(f,s,poles,weight,opts)
%function [SER,poles,rmserr,fit]=vectfit3_Ding(f,s,poles,weight)
%
%     ===========================================================
...
if (opts.relax~=0) && (opts.relax)~=1
    disp(['    ERROR in vectfit3_Ding.m: ==> Illegal value for opts.relax:  ' num2str(opts.asymp)]),return
end
if (opts.asymp~=1) && (opts.asymp)~=2 && (opts.asymp)~=3
    disp(['    ERROR in vectfit3_Ding.m: ==> Illegal value for opts.asymp:  ' num2str(opts.asymp)]),return
end
if (opts.stable~=0) && (opts.stable~=1)
    disp(['    ERROR in vectfit3_Ding.m: ==> Illegal value for opts.stable:  ' num2str(opts.stable)]),return
end
if (opts.skip_pole~=0) && (opts.skip_pole)~=1
    disp(['    ERROR in vectfit3_Ding.m: ==> Illegal value for opts.skip_pole:  ' num2str(opts.skip_pole)]),return
end
if (opts.skip_res~=0) && (opts.skip_res)~=1
    disp(['    ERROR in vectfit3_Ding.m: ==> Illegal value for opts.skip_res:  ' num2str(opts.skip_res)]),return
end
if (opts.cmplx_ss~=0) && (opts.cmplx_ss)~=1
    disp(['    ERROR in vectfit3_Ding.m: ==> Illegal value for opts.cmplx_ss:  ' num2str(opts.cmplx_ss)]),return
end
...
if length(s)~=length(f(1,:))
    disp('Error in vectfit3_Ding.m!!! ==> Second dimension of f does not match length of s.');
    return;
end
if length(s)~=length(weight(1,:))
    disp('Error in vectfit3_Ding.m!!! ==> Second dimension of weight does not match length of s.');
    return;
end
if length(weight(:,1))~=1
    if length(weight(:,1))~=length(f(:,1))
        disp('Error in vectfit3_Ding.m!!! ==> First dimension of weight is neither 1 nor matches first dimension of f.');
        return;
    end
end
...
elseif length(weight(1,:))==Nc
    ...
    disp('ERROR in vectfit3_Ding.m: Invalid size of array weight')
    return
end
