...
def Vector_Fitting_auto(f, s, n_poles=10, n_iter=10, show=False,
...
def Vector_Fitting_auto_rescale(f, s, **kwargs):
    s_scale = abs(s[-1])
    f_scale = abs(f[-1])
    print( 'SCALED')
    poles_s, residues_s, d_s, h_s = Vector_Fitting_auto(f / f_scale, s / s_scale, **kwargs)
...
if __name__ == '__main__':
    ...
    Vector_Fitting_auto(test_f, test_s)

    poles, residues, d, h = Vector_Fitting_auto_rescale(test_f, test_s)
