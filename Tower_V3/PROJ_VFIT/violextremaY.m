...
for m=1:length(wintervals(:,1))
    ...
    if wintervals(m,2)==1e16 %violation extends to infinite frequency
        w2=2*pi*1e16; %tvilsom måte ?spesifisere uendelig frekv. p?
    else
        w2=wintervals(m,2);
    end
    s_pass1=1i*linspace(w1,w2,Nint);
    if w1==0
        s_pass2=1i*logspace(-8,log10(w2),Nint);
    else
        s_pass2=1i*logspace(log10(w1),log10(w2),Nint);
    end
    s_pass=sort([s_pass1 s_pass2]); 
    Nint=2*Nint;
